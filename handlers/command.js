...
console.log("Welcome to HANDLER SERVICE // By kuka ")
    ...
    client.distube
        ...
        .on("playList", (message, queue, playlist, song) => {
            try {
                ...
                console.error(error)
                functions.embedbuilder(5000, message, config.colors.no, "**ERROR** <a:kuka_26:799630865474256972> ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
                functions.errorbuilder(error.stack.toString().substr(0, 2000))
            }
            try {
                queue.connection.voice.setSelfDeaf(true);
            ...
            functions.embedbuilder(client, 7500, message, config.colors.yes, "**Added a Playlist** <a:kuka_31:834550131658326055>", `Playlist: [\`${playlist.name}\`](${playlist.url ? playlist.url : ""})  -  \`${playlist.songs.length ? playlist.songs.length : "undefinied"} songs\` \n\nRequested by: ${queue.songs[0].user ? queue.songs[0].user : "error"}`, playlist.thumbnail.url ? playlist.thumbnail.url : "")

            try {
                functions.playsongyes(client, message, queue, queue.songs[0]);
            } catch (error) {
                console.error(error)
                functions.embedbuilder(client, 5000, message, "#ff264a", "**ERROR** <a:kuka_26:799630865474256972> ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
                functions.errorbuilder(error.stack.toString().substr(0, 2000))
            }
        })
        .on("addList", (message, queue, playlist) => {
            try {
                return functions.embedbuilder(client, 7500, message, config.colors.yes, "**Added a Playlist** <a:kuka_31:834550131658326055>", `Playlist: [\`${playlist.name}\`](${playlist.url ? playlist.url : ""})  -  \`${playlist.songs.length ? playlist.songs.length : "undefinied"} songs\` \n\nRequested by: ${queue.songs[0].user ? queue.songs[0].user : "error"}`, playlist.thumbnail.url ? playlist.thumbnail.url : "")
            } catch (error) {
                console.error(error)
                functions.embedbuilder(client, 5000, message, config.colors.no, "**ERROR** <a:kuka_26:799630865474256972> ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
                functions.errorbuilder(error.stack.toString().substr(0, 2000))
        ...
        .on("error", (message, err) => {
            try {
                message.reactions.removeAll();
                message.react("<a:kuka_26:799630865474256972>")
            } catch (error) {
                console.error(error)
                functions.embedbuilder(client, 5000, message, config.colors.no, "**ERROR**<a:kuka_26:799630865474256972> ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
                functions.errorbuilder(error.stack.toString().substr(0, 2000))
        ...
        .on("empty", message => {
            ...
            try {
                return functions.embedbuilder(client, 5000, message, config.colors.yes, "**Left the channel cause it got empty**<a:kuka_26:799630865474256972>")
            } catch (error) {
                console.error(error)
        ...
        .on("noRelated", message => {
            try {
                return functions.embedbuilder(client, 5000, message, config.colors.yes, "**Can't find related video to play. Stop playing music**<a:kuka_26:799630865474256972>")
            } catch (error) {
                console.error(error)
                functions.embedbuilder(client, 5000, message, config.colors.no, "**ERROR** <a:kuka_26:799630865474256972> ", "```" + error.toString().substr(0, 100) + "```" + "\n\n**Error got sent to my owner!**")
                functions.errorbuilder(error.stack.toString().substr(0, 2000))
